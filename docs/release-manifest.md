...
- `release_judgment`
  - `v0.2.0` 已完成执行层、模式层、初始 benchmark 与经验沉淀骨架，但验证体系仍在加固，尚未正式发布
...
当前迭代重点不是继续扩写概念，而是补三类发布前缺口：

1. 验证体系从回放式 benchmark 升级为 baseline + pairwise rubric + experiment log
2. `medium` 以上任务默认保留最小回收块，降低经验沉淀门槛
3. README 与 agent metadata 优先优化首次试用路径
